...
- [ ] 验证 T1 `AB5E0001` 服务的特征 UUID、能力协商、音频帧格式和采样率；探测和夹具工具已完成。
- [x] 已实现协议无关的 IMA-DVI ADPCM、PCM 管线和 WASAPI 输出端点；T1 是否兼容仍待真实报文确认。
...
13 个启用遥控按键的原始 HID 报文和 Usage 映射已经通过 GUI Raw Input 采集确认；Air Mouse 仍按规则不进入主动标注流程。语音部分需要另外确认 T1 是否真的传输音频，以及是否完全兼容参考项目的 ATVV 协议。
