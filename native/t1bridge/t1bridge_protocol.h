...
#define T1BRIDGE_MAX_REPORT_DESCRIPTOR_BYTES 4096u
#define T1BRIDGE_MAX_PREPARSED_DATA_BYTES 4096u
...
#define T1BRIDGE_CAPABILITY_REPORT_DESCRIPTOR 0x00000080u
#define T1BRIDGE_CAPABILITY_PREPARSED_DATA 0x00000100u

/*
 * 程序说明：按调用方句柄权限区分查询与修改。
 *
 * 官方要求管理类 IOCTL 不能使用 FILE_ANY_ACCESS：查询类要求 FILE_READ_DATA，
 * 会改变策略、过滤状态或租约的请求要求 FILE_WRITE_DATA。这样即使控制设备
 * ACL 放宽给普通用户只读权限，也只有持有写权限的句柄（管理员 App）能下发
 * 策略和启停过滤；配合控制会话归属，第二个句柄不能抢占正在运行的会话。
 */
...
#define IOCTL_T1FILTER_GET_REPORT_DESCRIPTOR \
    CTL_CODE(FILE_DEVICE_UNKNOWN, 0x809, METHOD_BUFFERED, FILE_READ_DATA)
#define IOCTL_T1FILTER_GET_PREPARSED_DATA \
...
typedef struct T1BRIDGE_DESCRIPTOR_QUERY {
    T1BRIDGE_UINT16 collection;
    T1BRIDGE_UINT16 reserved;
} T1BRIDGE_DESCRIPTOR_QUERY;

typedef struct T1BRIDGE_REPORT_DESCRIPTOR {
    T1BRIDGE_UINT32 size;
    T1BRIDGE_UINT32 abi_version;
    T1BRIDGE_UINT16 collection;
    T1BRIDGE_UINT16 reserved;
    T1BRIDGE_UINT32 descriptor_length;
    T1BRIDGE_UINT8 descriptor[T1BRIDGE_MAX_REPORT_DESCRIPTOR_BYTES];
} T1BRIDGE_REPORT_DESCRIPTOR;

typedef struct T1BRIDGE_PREPARSED_DATA {
    T1BRIDGE_UINT32 size;
    T1BRIDGE_UINT32 abi_version;
    T1BRIDGE_UINT16 collection;
    T1BRIDGE_UINT16 reserved;
    T1BRIDGE_UINT32 data_length;
    T1BRIDGE_UINT8 data[T1BRIDGE_MAX_PREPARSED_DATA_BYTES];
} T1BRIDGE_PREPARSED_DATA;
#pragma pack(pop)
...
__declspec(dllexport) T1BRIDGE_INT32 T1Bridge_GetReportDescriptor(
    void* handle,
    T1BRIDGE_REPORT_DESCRIPTOR* descriptor
);
__declspec(dllexport) T1BRIDGE_INT32 T1Bridge_GetPreparsedData(
    void* handle,
