...
- `tools.t1_voice_test`：连接真实 T1，完成 ATVV 能力协商、`MIC_OPEN`、ADPCM 解码、PCM 重采样和 VB-CABLE 输出；退出时按顺序关闭麦克风、断开 GATT、排空 PCM 输出。
- `tools.t1_app` 的“语音测试”页：启动时后台扫描 BLE 设备，并在广播扫描为空时回退读取 Windows 缓存的 T1-Remote；识别到 T1/Remote 后预填地址，也可以手动扫描并选择设备，再启动或停止真实语音会话、查看实时波形并播放最近一次录音。
